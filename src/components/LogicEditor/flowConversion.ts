...
export function convertBlocksToFlow(
  ...
  const existingComps = components.map((c) => ({
    id: c.id,
    name: c.name,
    type: c.type,
  }));
  ...
  const componentsList = [
    screenComp,
    ...existingComps.filter((c) => !c.id.startsWith("root") && c.name.toLowerCase() !== "screen1"),
  ];
  ...
  (logicBlocks || []).forEach((block, bIndex) => {
    ...
    (block.actions || []).forEach((action, aIndex) => {
      // If action has conditionEnabled or subActions branching
      if ((action.conditionEnabled && action.condition) || (action.subActions && action.subActions.length > 0)) {
...
export function getRecipePreset(
  recipeId: "youtube_flow" | "hello_world" | "login_flow" | "counter_flow",
  ...
  if (recipeId === "youtube_flow") {
    return {
      ...
      event: "Click",
      description: "Paste YouTube link or play embedded video in player",
      enabled: true,
      position: { x: 100, y: 60 },
      actions: [
        {
