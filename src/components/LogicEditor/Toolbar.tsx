    ...
    <header className="h-12 bg-slate-900 border-b border-slate-800 px-3 flex items-center justify-between gap-2 select-none text-slate-200 z-10">
      ...
      <div className="flex items-center gap-2 min-w-0">
        ...
        <div className="hidden md:flex items-center gap-1 text-[11px] text-slate-400 font-mono px-2 py-0.5 rounded-md bg-slate-950/60 border border-slate-800">
          ...
          <span>{totalBlocks} Trigger{totalBlocks === 1 ? "" : "s"}</span>
        </div>

        {/* Real-time Flow Validation Button */}
        {onToggleValidationDrawer && (
          <button
            type="button"
            onClick={onToggleValidationDrawer}
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-xl text-xs font-bold transition cursor-pointer border shadow-xs ${
              errorCount > 0
                ? "bg-rose-950/90 text-rose-300 border-rose-500/80 ring-2 ring-rose-500/30 animate-pulse"
                : warningCount > 0
                ? "bg-amber-950/90 text-amber-300 border-amber-500/80 hover:bg-amber-900/90"
                : "bg-emerald-950/60 text-emerald-300 border-emerald-500/40 hover:bg-emerald-900/60"
            }`}
            title="View Real-time Flow Static Analysis & Issue Drawer"
          >
            {errorCount > 0 ? (
              <>
                <div className="w-4 h-4 rounded-full bg-rose-600 text-white flex items-center justify-center text-[10px] font-mono">
                  {errorCount}
                </div>
