...
import { analyzeFlow, FlowValidationError } from "./flowValidator";
import { FlowValidationDrawer } from "./FlowValidationDrawer";
...
import { loadBuilderSettings } from "../../lib/builderSettings";
import { useVariablesWatch } from "./useVariablesWatch";
import { getAllComponentsFromScreen } from "../../utils/screenManager";
import {
  Sparkles,
  Zap,
  Plus,
...
// Define custom node types & edge types outside component to prevent re-creation loops
...
const STATIC_EDGE_TYPES = {
  ...
  step: InsertableEdge,
  straight: InsertableEdge,
  default: InsertableEdge,
  insertable: InsertableEdge,
};

const STATIC_DEFAULT_EDGE_OPTIONS = {
  type: "smoothstep",
  style: { stroke: "#60A5FA", strokeWidth: 2 },
};

const STATIC_SNAP_GRID: [number, number] = [20, 20];

const STATIC_PRO_OPTIONS = { hideAttribution: true };
...
const LogicFlowCanvas: React.FC<LogicFlowEditorProps> = ({
  ...
  // Extract all active UI components from screen.rootComponent tree
  const componentsOnScreen = useMemo(() => {
    return getAllComponentsFromScreen(screen);
  }, [screen.rootComponent, screen.id]);
  ...
  // Sync nodes and edges whenever screen.logicBlocks or screen.id changes (e.g. from Code Studio)
  useEffect(() => {
  ...
  // Keep latest refs to prevent re-render loops in callbacks
  ...
  // Flow Static Analysis & Validation Drawer state
  const [isValidationDrawerOpen, setIsValidationDrawerOpen] = useState(false);

  // Real-time Static Analysis of Flow Layout
  ...
  }, [nodes, edges, componentsOnScreen, screens, screen.stateVariables]);
  ...
  const nodeErrorMap = useMemo(() => {
    ...
    validationErrors.forEach((e) => {
      if (!map.has(e.nodeId) || e.severity === "error") {
        map.set(e.nodeId, e.title);
      }
    });
    return map;
  }, [validationErrors]);
  ...
  const displayNodes = useMemo(() => {
    return nodes.map((n) => {
      const errorText = nodeErrorMap.get(n.id);
      if (errorText) {
        return {
          ...n,
          data: {
  ...
  const errorNodeIds = useMemo(() => new Set(nodeErrorMap.keys()), [nodeErrorMap]);

  const activeSettings = builderSettings || loadBuilderSettings();

  const flowLineColorMap: Record<string, string> = {
    violet: "#8B5CF6",
    cyan: "#06B6D4",
    emerald: "#10B981",
    amber: "#F59E0B",
  ...
  const activeStrokeColor = flowLineColorMap[activeSettings.flowLineColor] || "#8B5CF6";
  const activeAnimationType = activeSettings.flowLineAnimation || "animated-dots";
  const activeEdgeType = activeSettings.flowLineStyle === "step" ? "step" : activeSettings.flowLineStyle === "straight" ? "straight" : "smoothstep";

  const displayEdges = useMemo(() => {
    return edges.map((e) => {
      const isBroken = errorNodeIds.has(e.source) || errorNodeIds.has(e.target);
      return {
        ...e,
        type: activeEdgeType,
        data: {
          ...e.data,
          isBroken,
          strokeColor: isBroken ? "#F43F5E" : activeStrokeColor,
          animationType: activeAnimationType,
          pulseColor: activeStrokeColor,
        },
        style: {
          ...e.style,
          stroke: isBroken ? "#F43F5E" : activeStrokeColor,
        },
      };
    });
  }, [edges, errorNodeIds, activeStrokeColor, activeAnimationType, activeEdgeType]);
  ...
  const persistFlowToScreen = useCallback(
    (currentNodes: FlowNode[], currentEdges: FlowEdge[]) => {
      ...
      syncTimeoutRef.current = setTimeout(() => {
        ...
        if (JSON.stringify(currScreen.logicBlocks) !== JSON.stringify(blocks)) {
          onUpdateScreen({
            ...currScreen,
            logicBlocks: blocks,
          });
  ...
  const handleFocusNodeOnCanvas = useCallback(
    (nodeId: string) => {
  ...
  const handleQuickFixError = useCallback(
    (error: FlowValidationError) => {
      if (error.quickFixType === "declare_variable" && error.targetName) {
        const varName = error.targetName;
        const exists = (screen.stateVariables || []).some((v) => v.name === varName);
        if (!exists) {
          ...
          const nextVars = [...(screen.stateVariables || []), newVar];
          onUpdateScreen({
            ...screen,
            stateVariables: nextVars,
          });
          setSimulationLog(`✓ Declared variable "${varName}" in State Variables`);
          setTimeout(() => setSimulationLog(null), 3000);
        }
      } else if (error.quickFixType === "fix_screen" && screens && screens.length > 0) {
        const firstScreenId = screens[0].id;
        setNodes((nds) => {
          const next = nds.map((n) => {
            if (n.id === error.nodeId && n.type === "action") {
              const act = (n.data as any).action;
              return {
                ...n,
                data: {
                  ...n.data,
        ...
        setSimulationLog(`✓ Updated navigation target screen to "${screens[0].name}"`);
        setTimeout(() => setSimulationLog(null), 3000);
      }
    },
    [screen, screens, edges, onUpdateScreen, persistFlowToScreen, setNodes]
  );

  const handleChangeComponentTarget = useCallback(
    (nodeId: string) => {
      const firstComp = componentsOnScreen[0];
      if (!firstComp) return;
      setNodes((nds) => {
        const next = nds.map((n) => {
          if (n.id === nodeId) {
      ...
      setSimulationLog(`✓ Re-linked component target to "${firstComp.name}" (${firstComp.id})`);
      setTimeout(() => setSimulationLog(null), 3000);
    },
    [componentsOnScreen, edges, persistFlowToScreen, setNodes]
  );
  ...
  const executeDeleteCascade = useCallback(
    (nodeId: string) => {
      ...
      const nextNodes = nodes.filter((n) => !downstreamIds.has(n.id));
      const nextEdges = edges.filter((e) => !downstreamIds.has(e.source) && !downstreamIds.has(e.target));
      setNodes(nextNodes);
      setEdges(nextEdges);
      persistFlowToScreen(nextNodes, nextEdges);
      setDeleteConfirm((prev) => ({ ...prev, isOpen: false }));
    },
    [nodes, edges, getDownstreamNodeIds, persistFlowToScreen]
  );
  ...
  // Persistence callback when dragging nodes stops on canvas
  const onNodeDragStop = useCallback(
    () => {
      setNodes((currentNodes) => {
        persistFlowToScreen(currentNodes as FlowNode[], edges);
        return currentNodes;
      });
    },
    [edges, persistFlowToScreen]
  );

  // Deletion handler when pressing Delete key or using ReactFlow delete action
  ...
  const requestDeleteNode = useCallback(
    (nodeId: string) => {
      ...
      const downstreamIds = getDownstreamNodeIds(nodeId, edgesRef.current);
    ...
    [getDownstreamNodeIds, executeDeleteSingle]
  );
  ...
  useEffect(() => {
    (window as any).__onFlowUpdateNode = (nodeId: string, patchData: any) => {
      setNodes((nds) => {
        ...
        persistFlowToScreen(next as FlowNode[], edgesRef.current);
        return next;
      });
    };
    ...
    (window as any).__onFlowDuplicateNode = (nodeId: string) => {
      ...
      const next = [...currentNodes, duplicate];
      setNodes(next);
      persistFlowToScreen(next as FlowNode[], edgesRef.current);
    };
    ...
    (window as any).__onFlowAddConnectedAction = (
      ...
      if (sourceHandleId === "branch-yes" || sourceHandleId === "branch-success") {
        newX = sourceNode.position.x - 160;
        newY = sourceNode.position.y + 220;
      } else if (sourceHandleId === "branch-no" || sourceHandleId === "branch-failure") {
        newX = sourceNode.position.x + 160;
        newY = sourceNode.position.y + 220;
      }
      ...
      const newAction: LogicAction = {
        ...
        targetId: (currentScreen.components || [])[0]?.id || "widget",
        property: "text",
        value: "New Action",
      };
      ...
      const newActionNode: FlowNode = {
        ...
        data: {
          ...
          actionId: actId,
          action: newAction,
          componentsList: (currentScreen.components || []).map((c) => ({
            id: c.id,
            name: c.name,
            type: c.type,
          })),
          ...
          stateVariables: currentScreen.stateVariables || [],
        },
      };
      ...
      const nextNodes = [...currentNodes, newActionNode];
    ...
    return () => {
      ...
      delete (window as any).__onFlowInsertOnEdge;
    };
  }, [requestDeleteNode, screens, persistFlowToScreen]);
  ...
  const onConnect = useCallback(
    (connection: Connection) => {
      ...
      setEdges((eds) => {
        ...
        persistFlowToScreen(nodesRef.current as FlowNode[], next);
        return next;
      });
    },
    [persistFlowToScreen]
      ...
      if (type === "trigger") {
        ...
        newNode = {
          ...
          data: {
            ...
            enabled: true,
            componentsList: componentsOnScreen.map((c) => ({
              id: c.id,
              name: c.name,
              type: c.type,
            })),
        ...
        newNode = {
          ...
          data: {
            ...
            action: {
              ...
              targetId: payload.targetId || (screen.components || [])[0]?.id || "root",
              property: payload.property || "text",
              value: payload.value ?? (payload.actionType === "callMethod" ? undefined : "Hello World"),
              methodName: payload.methodName,
              methodArgs: payload.methodArgs || (payload.methodName === "SeekTo" ? { position: 30.0 } : payload.methodName === "Load" || payload.methodName === "InstantLoad" || payload.methodName === "GetThumbnailFromVideoId" ? { videoId: "dQw4w9WgXcQ" } : {}),
            ...
            componentsList: [
              { id: "root", name: "Screen1", type: "Screen" },
  ...
  const handleApplyRecipe = useCallback(
    (recipeId: "hello_world" | "login_flow" | "counter_flow") => {
      const presetBlock = getRecipePreset(recipeId, targetComponentId || undefined);
      const existingBlocks = screen.logicBlocks || [];
      const newBlocks = [...existingBlocks, presetBlock];
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-slate-950 overflow-hidden relative">
      ...
      <Toolbar
        ...
        onToggleValidationDrawer={() => setIsValidationDrawerOpen((prev) => !prev)}
      />

      {/* Main Flow Editor Workspace: Left Sidebar + Canvas */}
      <div className="flex-1 flex h-[calc(100%-48px)] overflow-hidden relative">
        {/* Left Toolbox */}
        <Toolbox
          components={componentsOnScreen}
          screens={screens || []}
          screen={screen}
        ...
        >
          <NodeErrorContext.Provider value={nodeErrorMap}>
            <ReactFlow
              nodes={nodes}
              edges={edges}
              ...
              minZoom={0.2}
              maxZoom={2}
              defaultEdgeOptions={STATIC_DEFAULT_EDGE_OPTIONS}
              proOptions={STATIC_PRO_OPTIONS}
              className="bg-slate-950"
            >
              {showGrid && <Background color="#334155" gap={20} size={1} />}
              ...
              <MiniMap
                nodeColor={(n) => {
                  ...
                  if (n.type === "apiBranch") return "#38BDF8";
                  if (n.type === "note") return "#FBBF24";
                  return "#3B82F6";
                }}
                className="!bg-slate-900 !border-slate-800 !rounded-xl overflow-hidden shadow-lg hidden sm:block"
              />
            </ReactFlow>
          </NodeErrorContext.Provider>
          ...
          {/* Real-time Flow Validation & Issue Drawer */}
          <FlowValidationDrawer
            isOpen={isValidationDrawerOpen}
            onClose={() => setIsValidationDrawerOpen(false)}
            errors={validationErrors}
            onFocusNode={handleFocusNodeOnCanvas}
            onQuickFix={handleQuickFixError}
            onChangeComponentTarget={handleChangeComponentTarget}
          />
...
export const LogicFlowEditor: React.FC<LogicFlowEditorProps> = (props) => {
  return (
    <ReactFlowProvider>
      <LogicFlowCanvas key={`${props.screen?.id || "screen"}_${props.screen?.name || "main"}`} {...props} />
    </ReactFlowProvider>
  );
};
