...
export const ThemeManagementPanel: React.FC<ThemeManagementPanelProps> = ({
  ...
  // Extract uploaded font assets
  const customFontAssets = (assets || []).filter(
    (a) =>
      a.type === "font" ||
  ...
  const customFontOptions = customFontAssets.map((a) => {
    const cleanName = a.name.replace(/\.[^/.]+$/, "").replace(/[^a-zA-Z0-9_]/g, "_").toLowerCase();
    return {
      name: cleanName,
      displayName: a.name,
      category: "Uploaded Asset (res/font)",
      style: `Custom project font asset (${a.fileName})`,
      isCustom: true,
    };
  });

  const combinedFontOptions = [
    ...customFontOptions,
    ...FONT_OPTIONS.map((f) => ({ ...f, displayName: f.name, isCustom: false })),
  ];
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        ...
                        showNotification(`Set global font to "${font.displayName}"`);
                      }}
                    ...
                    >
                      <div className="flex items-center justify-between w-full">
                        <div className="flex items-center gap-2 truncate">
                          ...
                          {font.isCustom && (
                            <span className="text-[10px] font-mono px-1.5 py-0.5 rounded bg-amber-100 text-amber-800 border border-amber-300 shrink-0">
                              Custom Font
                            </span>
                          )}
                        </div>
