...
/**
 * Traverses screen.rootComponent recursively and extracts all active components placed on the screen.
 */
export function getAllComponentsFromScreen(screen: AndroidScreen): AndroidComponent[] {
  if (!screen) return [];
  const list: AndroidComponent[] = [];
  const visited = new Set<string>();

  const traverse = (comp: AndroidComponent) => {
    if (!comp || visited.has(comp.id)) return;
    visited.add(comp.id);
    if (comp.id !== "root" && comp.type !== "Screen") {
      list.push(comp);
    }
    if (comp.children && Array.isArray(comp.children)) {
