    ...
    >
      ...
      {activeTab === "theme" && (
        <div style={ideShellStyle} className="ide-theme-container flex-1 overflow-hidden h-full flex flex-col">
          <ThemeManagementPanel config={config} onUpdateConfig={onUpdateConfig} />
        </div>
      )}
      ...
      {["general", "branding", "splash", "monetization", "services", "manifest"].includes(activeTab) && (
        ...
        {activeTab === "general" && (
          <div className="max-w-4xl space-y-6 animate-in fade-in duration-150">
            ...
            {/* Language & Framework Architecture Configuration */}
            <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-5">
              <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                <div>
                  <h3 className="text-sm font-bold text-white flex items-center gap-2">
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    Select your preferred Android project architecture: Kotlin + Jetpack Compose or Java + XML Layouts.
                  </p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                >
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Modern Google-recommended Android UI toolkit. Uses declarative Kotlin Composables, Material 3, ViewModel, and Kotlin DSL Gradle (`build.gradle.kts`).
                    </p>
                  </div>
                  <div className="mt-3 pt-2 border-t border-slate-800/80 flex items-center gap-2 text-[10px] font-mono text-violet-300">
                    <span className="px-1.5 py-0.5 rounded bg-violet-900/60 border border-violet-700/50">Kotlin 2.0+</span>
                    <span className="px-1.5 py-0.5 rounded bg-violet-900/60 border border-violet-700/50">Compose 1.6+</span>
                    <span className="px-1.5 py-0.5 rounded bg-violet-900/60 border border-violet-700/50">Modern</span>
                  </div>
                </div>

                {/* Java Option */}
                <div
                  onClick={() => onUpdateConfig({ language: "java" })}
                  className={`p-4 rounded-xl border cursor-pointer transition relative flex flex-col justify-between ${
                ...
                >
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-400 leading-relaxed">
                      Classic Android SDK architecture. Uses `AppCompatActivity`, `setContentView(R.layout.activity_main)`, XML layouts in `res/layout/`, and Java source files.
                    </p>
                  </div>
                  <div className="mt-3 pt-2 border-t border-slate-800/80 flex items-center gap-2 text-[10px] font-mono text-amber-300">
                    <span className="px-1.5 py-0.5 rounded bg-amber-900/60 border border-amber-700/50">Java 17/21</span>
                    <span className="px-1.5 py-0.5 rounded bg-amber-900/60 border border-amber-700/50">XML Views</span>
                    <span className="px-1.5 py-0.5 rounded bg-amber-900/60 border border-amber-700/50">AppCompat</span>
                  </div>
                </div>
              </div>
              ...
              {config.language === "java" && (
                <div className="space-y-2 pt-2 border-t border-slate-800">
                  <label htmlFor="java-version-select" className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                    Java Version Toolchain
                  </label>
                  <select
                    id="java-version-select"
                    value={config.javaVersion || "17"}
                    onChange={(e) => onUpdateConfig({ javaVersion: e.target.value as "17" | "21" | "11" | "8" })}
                    className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-sm text-white focus:outline-none focus:border-amber-500 transition font-medium"
                  >
                    <option value="17">Java 17 (LTS - Standard for Android Gradle Plugin 8.x)</option>
                    <option value="21">Java 21 (LTS - Modern Latest)</option>
                    <option value="11">Java 11 (Legacy LTS)</option>
                    <option value="8">Java 1.8 (Classic Android Backward Compatibility)</option>
                  </select>
                </div>
              )}
            </div>
                ...
                <div className="md:col-span-7 space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-white focus:outline-none focus:border-violet-500 font-mono"
                    />
                    <p className="text-[11px] text-slate-400">
