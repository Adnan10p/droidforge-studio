...
import { X, Sparkles, Layers, ArrowRight, Check, Code2 } from "lucide-react";
...
export const TemplatesModal: React.FC<TemplatesModalProps> = ({
  ...
  const [selectedLanguage, setSelectedLanguage] = useState<"kotlin" | "java">("kotlin");
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4 animate-in fade-in duration-200">
      ...
      >
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Kickstart your application with pre-built architectures
              </p>
            </div>
          </div>
          <button
        ...
        <div className="px-6 py-3 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3 bg-white">
          ...
          {/* Language Architecture Toggle */}
          <div className="flex items-center gap-1.5 bg-slate-100 p-1 rounded-xl border border-slate-200 text-xs">
            <span className="text-[11px] font-semibold text-slate-500 px-1">Language:</span>
            <button
              type="button"
              onClick={() => setSelectedLanguage("kotlin")}
              className={`px-2.5 py-0.5 rounded-lg text-xs font-bold transition flex items-center gap-1 ${
