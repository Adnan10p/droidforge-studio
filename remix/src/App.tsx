...
import { LiveCompanionView } from "./components/Companion/LiveCompanionView";
import { companionBridge } from "./components/Companion/companionBridge";
import { CompanionPayload } from "./types";
...
export default function App() {
  ...
  const [isCompanionStandalone] = useState<boolean>(() => {
    try {
      return typeof window !== "undefined" && new URLSearchParams(window.location.search).get("companion") === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    // High Contrast Logic custom CSS variables on canvas & root
    const isHighContrastLogic =
      builderSettings.theme === "high-contrast-logic" ||
      builderSettings.theme === "high-contrast" ||
      (isCustom && (Boolean(custom.logicCanvasBg) || custom.accentColor === "#38BDF8"));

    const logicCanvasBg =
      isCustom && custom.logicCanvasBg
        ? custom.logicCanvasBg
        : isHighContrastLogic
        ? "#050811"
        : isDark
        ? "#0B0F19"
    ...
    const logicGridColor =
      isCustom && custom.logicGridColor
        ? custom.logicGridColor
        : isHighContrastLogic
        ? "#334155"
        : isDark
        ? "#1E293B"
    ...
    const logicEdgeStroke =
      isCustom && custom.logicEdgeColor
        ? custom.logicEdgeColor
        : isHighContrastLogic
        ? "#38BDF8"
        : "#60A5FA";

    const logicEdgeWidth = isHighContrastLogic ? "3px" : "2.5px";

    const logicTriggerBorder =
      isCustom && custom.logicTriggerBorder
        ? custom.logicTriggerBorder
        : isHighContrastLogic
        ? "#10B981"
        : "#059669";

    const logicTriggerBg = isHighContrastLogic ? "#061F16" : "#0F172A";

    const logicActionBorder =
      isCustom && custom.logicActionBorder
        ? custom.logicActionBorder
        : isHighContrastLogic
        ? "#3B82F6"
        : "#2563EB";

    const logicActionBg = isHighContrastLogic ? "#0A172E" : "#0F172A";

    const logicConditionBorder =
      isCustom && custom.logicConditionBorder
        ? custom.logicConditionBorder
        : isHighContrastLogic
        ? "#F59E0B"
        : "#D97706";

    const logicConditionBg = isHighContrastLogic ? "#221403" : "#0F172A";

    const logicCommentBorder =
      isCustom && custom.logicCommentBorder
        ? custom.logicCommentBorder
        : isHighContrastLogic
        ? "#8B5CF6"
        : "#7C3AED";

    const logicCommentBg = isHighContrastLogic ? "#170C29" : "#0F172A";
    ...
    targets.forEach((el) => {
      ...
      // Logic Blocks Canvas Variables
      el.style.setProperty("--logic-canvas-bg", logicCanvasBg);
      el.style.setProperty("--logic-canvas-grid", logicGridColor);
      el.style.setProperty("--logic-edge-stroke", logicEdgeStroke);
      el.style.setProperty("--logic-edge-width", logicEdgeWidth);
      el.style.setProperty("--logic-trigger-border", logicTriggerBorder);
      el.style.setProperty("--logic-trigger-bg", logicTriggerBg);
      el.style.setProperty("--logic-action-border", logicActionBorder);
      el.style.setProperty("--logic-action-bg", logicActionBg);
      el.style.setProperty("--logic-condition-border", logicConditionBorder);
      el.style.setProperty("--logic-condition-bg", logicConditionBg);
      el.style.setProperty("--logic-comment-border", logicCommentBorder);
      el.style.setProperty("--logic-comment-bg", logicCommentBg);
      el.style.setProperty("--logic-node-contrast", isHighContrastLogic ? "high" : "standard");
    });
  ...
  // Auto-sync active screen payload to Companion Bridge for real-time live testing
  useEffect(() => {
    ...
    companionBridge.pushPayload(payload, false);
  }, [currentScreen, config, currentProjectId, screens]);
  ...
  // Standalone Companion Receiver View for mobile browser pairing
  if (isCompanionStandalone) {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-4">
        <LiveCompanionView
          standalone
          payload={companionBridge.getLastPayload()}
          onRefreshScreen={() => companionBridge.refreshCompanionScreen()}
          onResetConnection={() => companionBridge.resetConnection()}
        />
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <Navbar
        ...
        onToggleLanguage={(lang) => handleUpdateConfig({ language: lang })}
        onOpenCompanionModal={() => setIsCompanionModalOpen(true)}
        onRefreshCompanionScreen={() => companionBridge.refreshCompanionScreen()}
        onResetCompanionConnection={() => companionBridge.resetConnection()}
      />
      ...
      {/* Live Test Companion Modal (Wi-Fi QR, USB ADB, Hot-Reload, Logs) */}
      <CompanionModal
