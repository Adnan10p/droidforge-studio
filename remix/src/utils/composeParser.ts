...
export function parseComposeToScreen(
  ...
  try {
    ...
    const createLogicBlockForAction = (
      ...
      // Check for Paste & YouTube Play flow
      const hasPasteMode = lambdaCode.includes("isPasteMode") || (comp.props.text || "").toLowerCase().includes("paste");
      const hasYouTubePlay = lambdaCode.includes("embed") || lambdaCode.includes("youtube") || lambdaCode.includes("activeEmbedUrl") || lambdaCode.includes("extractYouTubeVideoId");

      if (hasPasteMode || hasYouTubePlay) {
        actions.push({
          id: generateUniqueComposeId("cond_paste_mode"),
          ...
          subActions: [
            ...
            {
              id: generateUniqueComposeId("act_btn_play_lbl"),
              actionType: "setProperty",
              property: "text",
              value: "▶ Play YouTube Video",
            },
            {
              id: generateUniqueComposeId("act_btn_color_red"),
              actionType: "setProperty",
              property: "backgroundColor",
              value: "#DC2626",
            },
            {
              id: generateUniqueComposeId("act_set_paste_false"),
              actionType: "setVariable",
          ...
          elseActions: [
            ...
            {
              id: generateUniqueComposeId("act_toast_streaming"),
              actionType: "toast",
...
function parseChildElements(
  ...
  const btnKeywords = ["Button", "ElevatedButton", "OutlinedButton", "TextButton", "IconButton"];
  
  // Find button invocations with balanced parentheses and trailing lambda braces
  ...
  while ((btnCallMatch = btnStartRegex.exec(snippet)) !== null) {
    ...
    if (braceMatch) {
      ...
      while (j < snippet.length && braceDepth > 0) {
        const ch = snippet[j];
        if (inString) {
          if (ch === "\\") {
            j += 2;
      ...
      if (braceDepth === 0) {
        btnBody = snippet.substring(openBraceIndex + 1, j - 1);
        fullEndIndex = j;
      }
    }
    ...
    const lambdaCode = onClickMatch ? onClickMatch[1] : btnArgs;
  ...
  // 3. Scan for Text: Text(...) (excluding those inside Buttons or TextFields)
  ...
  while ((txtMatch = textRegex.exec(textScanSnippet)) !== null) {
    ...
    // Filter out common dummy placeholder texts
  ...
  // 10. Scan for WebView / AndroidView / Video
  const webViewRegex = /(?:AndroidView|WebView|VideoView)\s*[\(\{]/;
  if (webViewRegex.test(snippet)) {
    const isYouTube = /youtube|embed|videoUrl|activeEmbedUrl/i.test(snippet);
    const webViewComp: AndroidComponent = {
      id: generateUniqueComposeId("comp_webview"),
      type: "WebView",
      name: isYouTube ? "YouTubePlayer" : `WebView${nextCounter()}`,
      category: "Content",
      props: {
        ...getDefaultProps("WebView"),
        url: isYouTube ? "https://www.youtube.com/embed/dQw4w9WgXcQ" : "https://android.com",
    ...
    details.push(`Added ${webViewComp.name} player`);
  }
}
